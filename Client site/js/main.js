(function() {
	...
	var menuHover = function() {
		if (!isMobile.any()) {
			jQuery('.navbar-nav .navbar-nav li.dropdown').hover(function() {
	...
	var showcase = function() {
		...
		jQuery('.app-showcase-nav ul li a').on('click', function(e){

			var $this = jQuery(this),
					index = $this.closest('li').index();
							
			$this.closest('.app-feature-showcase').find('.app-showcase-nav ul li').removeClass('active');
			$this.closest('li').addClass('active');

			$this.closest('.app-feature-showcase').find('.app-images-list li').removeClass('active');
			$this.closest('.app-feature-showcase').find('.app-images-list li').eq(index).addClass('active');
	...
	var contentWayPoint = function() {
		var i = 0;
		jQuery('.app-animate').waypoint( function( direction ) {

			if( direction === 'down' && !jQuery(this.element).hasClass('app-animated') ) {
				...
				setTimeout(function(){
					...
					jQuery('body .app-animate.item-animate').each(function(k){
						var el = jQuery(this);
						setTimeout( function () {
							var effect = el.data('animate-effect');
							if ( effect === 'fadeIn') {
								el.addClass('fadeIn app-animated');
							} else if ( effect === 'fadeInLeft') {
								el.addClass('fadeInLeft app-animated');
							} else if ( effect === 'fadeInRight') {
								el.addClass('fadeInRight app-animated');
							} else {
								el.addClass('fadeInUp app-animated');
							}
	...
	var counterWayPoint = function() {
		if (jQuery('#app-counter').length > 0 ) {
			jQuery('#app-counter').waypoint( function( direction ) {
										
				if( direction === 'down' && !jQuery(this.element).hasClass('app-animated') ) {
					setTimeout( counter , 400);					
					jQuery(this.element).addClass('app-animated');
				}
			} , { offset: '90%' } );
		}
	};
	...
	jQuery(document).ready(function(){
		...
		showcase();
		contentWayPoint();
		if (jQuery('.app-gallery').length > 0) {
			initPhotoSwipeFromDOM('.app-gallery');
		}
		galleryMasonry();
		counterWayPoint();
		tabsOwl();
